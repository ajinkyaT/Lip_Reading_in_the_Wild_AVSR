...
while True:
	...
	l2_dist = [face[1] for face in faces[frame_num]]
	if len(l2_dist)!= 0: # think of case when the track has only one face and the person is not speaking try adding threshold for l2
		speaker = l2_dist.index(min(l2_dist))
		cv2.rectangle(image,(int(faces[frame_num][speaker][3]-faces[frame_num][speaker][2]),int(faces[frame_num][speaker][4]-faces[frame_num][speaker][2])),(int(faces[frame_num][speaker][3]+faces[frame_num][speaker][2]),int(faces[frame_num][speaker][4]+faces[frame_num][speaker][2])),(255,0,0),3)
		cv2.putText(image,'Track %d, L2 Dist %.3f'%(faces[frame_num][speaker][0],faces[frame_num][speaker][1]),(int(faces[frame_num][speaker][3]-faces[frame_num][speaker][2]),int(faces[frame_num][speaker][4]-faces[frame_num][speaker][2])),cv2.FONT_HERSHEY_SIMPLEX,0.5,(255,255,255),2)

	for face in [x for i,x in enumerate(faces[frame_num]) if i!=speaker]:
...
command = ("ffmpeg -y -i %s -i %s -c:v copy -c:a copy %s" % (os.path.join(opt.avi_dir,opt.reference,'video_only.avi'),os.path.join(opt.avi_dir,opt.reference,'audio_only.avi'),os.path.join(opt.avi_dir,opt.reference,'video_out_speaker.avi'))) #-async 1 
output = subprocess.call(command, shell=True, stdout=None)
